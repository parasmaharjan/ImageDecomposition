...
for epoch in range(0, 2001):
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        if 0:
            in_img_lh = torch.cat((in_img_ll, in_img_lh), dim=1)
            out_img_lh = model_lh(in_img_lh)
            in_img_hl = torch.cat((in_img_ll, in_img_hl), dim=1)
...
print('done training...')
# save model
torch.save(model_ll.state_dict(), model_dir + 'Wavelet_ll_add_original_sony_e%04d.pth' % epoch)
...
LL = np.minimum(np.maximum(LL, 0), 1)
LH = np.minimum(np.maximum(LH, 0), 1)
HL = np.minimum(np.maximum(HL, 0), 1)
HH = np.minimum(np.maximum(HH, 0), 1)
combine_img = Wavelet.combine3ch(LL, LH, HL, HH)
plt.figure(2)
